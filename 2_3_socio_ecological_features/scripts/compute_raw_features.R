...
extract_social <- function(sub, gps_data, time_option){
  ...
  # GPS data of other participants. 
...
# extract alternative roughness (variation depth within radius)
...
# extract alternative roughness 2 (max-min depth within radius)
...
# extract alternative roughness 3 (Average dist. to curves within radius)
...
if (file.exists("utils/data/processed_data/alternative_roughness_distance_matrix.rds")){
  ...
  # extract feature
  #alternative_roughness_distance_matrix = map(Feature_matrix, function(x) extract_ecological_features(x, depth_profiles, lake_key), .progress = TRUE)
