...
prepare_stan_data_spot_selection <- function(){
  ...
  # Helper Functions
  ################################################################################
  # in stan, all matrices in array need to have same dimensions, thus some matrices
  # need to be filled.
  ...
  prepare_stan_data <- function(social_distances,
    ...
    # join back (keep for later matching participant ids/lake ids back to model ests)
    ...
    # compute success columns (how many columns need to be subset within model)
    ...
    # select closest ncol spots/ids
    for (i in 1:length(social_distances)){
      
      # social
      social_distances[[i]][is.na(social_distances[[i]])] <- 0.0001 #missings happen rarely and bc. of low dist (<.001%)
    ...
    success_feature <- aperm(success_feature, c(3,1,2)) #dim 1: time, dim2: alternative spots, dim3: distances to success spots
    
    loss_feature <- abind(loss_distances, along=3)
    loss_feature <- aperm(loss_feature, c(3,1,2)) #dim 1: time, dim2: alternative spots, dim3: distances to unsuccessful spots
    
    roughness_feature <- abind(roughness_distances, along=3)
    roughness_feature <- aperm(roughness_feature, c(3,1,2)) #dim 1: time, dim2: alternative spots, dim3: distances to depth curves
  ...
  lake_variables$day = as.character(lake_variables$day)
  lake_variables$year = as.character(lake_variables$year)
