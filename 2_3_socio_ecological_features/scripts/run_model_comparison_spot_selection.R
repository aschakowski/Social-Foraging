...
stan_data$Distances_social[stan_data$Distances_social == 0] <- .001
stan_data$Distances_roughness[stan_data$Distances_roughness == 0] <- .001
stan_data$Distances_success[stan_data$Distances_success == 0] <- .001
stan_data$Distances_loss[stan_data$Distances_loss == 0] <- .001
stan_data$Locality[stan_data$Locality == 0] <- .001

# any missings in dataset? (n = 0)
...
# of alternative spots. Model was fit multiple times to ensure that the direction, size and credibility
# of effects is not affected by this randomness.
for (i in 1:length(model_locations)){
  
  if (i == 1){
    
    # fit random model using fixed params.
