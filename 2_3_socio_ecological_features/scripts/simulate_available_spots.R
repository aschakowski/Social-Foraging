...
simulate_available_spots <- function(spot_selection_data, N_spots, gps_data, point_lakes, competition_area){
  ...
  for (t in 1:T){
    ...
    # if first spot of foraging trip, then randomyl sample from wholel lake (not used in paper)
    if (t == 1 || chosen_spot$track_id != spot_selection_data[t-1, "track_id"]){
      ...
      if(chosen_spot$spot_id == 2){
        ...
        # starting point is the location where everyone started searching for initial spot.
        ...
        # if spot id >=3 heading angle can be computed.
      ...
      # simulate turning angle from empirical turning angle distribution (turning angles are changes in heading)
      turning_angle = sample(angles, N_spots, replace = T)
      ...
      for (pt in 1:length(simulated_x)){
        counter = 1
        while(!pnpoly(c(simulated_x[pt], simulated_y[pt]), as.matrix(lake_points)) & counter < 500){
          a = sample(angles, 1)
          r = sample(steps, 1)
    ...
    features <- features[(N_spots+1):1,] #resort to put chosen location in row 1
