...
spot_selection_data$unique_ID = paste(spot_selection_data$day, spot_selection_data$year, spot_selection_data$ID, sep = "")
...
scale_features = fitted_features %>% 
  ...
  summarize(sd_social = sd(social_feature),
            ...
            sd_locality_time = sd(locality_feature) * sd(time_since_start),
...
model_results = lake_effects %>% 
  ...
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Successful"], .17), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Successful"], .83), ymin = 3.65, ymax = 4.35,
           alpha = 1,fill = "#31A354")+
  # .5
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Successful"], .485), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Successful"], .515), ymin = 3.65, ymax = 4.35,
           alpha = 1,fill = "black")+
  ...
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social"], .17), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social"], .83),ymin = 1.65, ymax = 2.35,
           alpha = 1,fill = "#31A354")+
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social"], .49), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social"], .51),ymin = 1.65, ymax = 2.35,
           alpha = 1,fill = "black")+
  ...
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Roughness"], .17), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Roughness"], .83), ymin = 0.65, ymax = 1.35,
           alpha = 1,fill = "#31A354")+
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Roughness"], .49), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Roughness"], .51),ymin = 0.65, ymax = 1.35,
           alpha = 1,fill = "black")+
  ...
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Unsuccessful"], .17), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Unsuccessful"], .83), ymin = 2.65, ymax = 3.35,
           alpha = 1,fill = "#31A354")+ 
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Unsuccessful"], .49), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Unsuccessful"], .51), ymin = 2.65, ymax = 3.35,
           alpha = 1,fill = "black")+
  ylab("Feature")+
...
model_results_interaction = lake_effects %>% 
  ...
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social x Unsuccessful"], .17), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social x Unsuccessful"], .83), ymin = .65, ymax = 1.35,
           alpha = 1,fill = "#31A354")+
  # .5
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social x Unsuccessful"], .49), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social x Unsuccessful"], .51), ymin = .65, ymax = 1.35,
           alpha = 1,fill = "black")+
  ...
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social x Successful"], .17), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social x Successful"], .83),  ymin = 1.65, ymax = 2.35,
           alpha = 1,fill = "#31A354")+
  annotate("rect", xmin = quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social x Successful"], .49), xmax =  quantile(lake_effects$standardized_main_effect[lake_effects$feature == "Social x Successful"], .51),  ymin = 1.65, ymax = 2.35,
           alpha = 1,fill = "black")+
  stat_pointinterval(position = position_dodge(width = .8), color = "black", alpha = .10, size = 0)  +
...
bandwidth_results = bandwidths_offset_full %>% 
  ...
  annotate("rect", xmin = quantile(exp(bandwidths_full$lambdas[bandwidths_full$feature == "Personal\nInformation"]), .17), xmax =  quantile(exp(bandwidths_full$lambdas[bandwidths_full$feature == "Personal\nInformation"]), .83), ymin = 1.65, ymax = 2.35,
           alpha = 1,fill = "#31A354")+
  # .5
  annotate("rect", xmin = quantile(exp(bandwidths_full$lambdas[bandwidths_full$feature == "Personal\nInformation"]), .485), xmax =  quantile(exp(bandwidths_full$lambdas[bandwidths_full$feature == "Personal\nInformation"]), .515), ymin = 1.65, ymax = 2.35,
           alpha = 1,fill = "black")+
  ...
  annotate("rect", xmin = quantile(exp(bandwidths_full$lambdas[bandwidths_full$feature == "Social\nInformation"]), .17), xmax =  quantile(exp(bandwidths_full$lambdas[bandwidths_full$feature == "Social\nInformation"]), .83), ymin = 0.65, ymax = 1.35,
           alpha = 1,fill = "#31A354")+
  annotate("rect", xmin = quantile(exp(bandwidths_full$lambdas[bandwidths_full$feature == "Social\nInformation"]), .49), xmax =  quantile(exp(bandwidths_full$lambdas[bandwidths_full$feature == "Social\nInformation"]), .51),  ymin = 0.65, ymax = 1.35,
           alpha = 1,fill = "black")+
...
roughness = model %>%
  ...
  mutate(or = r_lake + b_roughness_scaled)
social = model %>%
  spread_draws(b_success_scaled,
               b_social_scaled,
               b_roughness_scaled,
...
adaptive_cues =
  ...
  annotate("rect", xmin = quantile(main_effect$b_success_scaled, .17), xmax =  quantile(main_effect$b_success_scaled, .83), ymin = 3.65, ymax = 4.35,
           alpha = 1,fill = "#31A354")+
  # .5
  annotate("rect", xmin = quantile(main_effect$b_success_scaled, .495), xmax =  quantile(main_effect$b_success_scaled, .505), ymin = 3.65, ymax = 4.35,
           alpha = 1,fill = "black")+
  ...
  annotate("rect", xmin = quantile(main_effect$b_social_scaled, .17), xmax =  quantile(main_effect$b_social_scaled, .83), ymin = 1.65, ymax = 2.35,
           alpha = 1,fill = "#31A354")+
  annotate("rect", xmin = quantile(main_effect$b_social_scaled, .49), xmax =  quantile(main_effect$b_social_scaled, .51), ymin = 1.65, ymax = 2.35,
           alpha = 1,fill = "black")+
  ...
  annotate("rect", xmin = quantile(main_effect$b_roughness_scaled, .17), xmax =  quantile(main_effect$b_roughness_scaled, .83),ymin = 0.65, ymax = 1.35,
           alpha = 1,fill ="#31A354") +
  annotate("rect", xmin = quantile(main_effect$b_roughness_scaled, .495), xmax =  quantile(main_effect$b_roughness_scaled, .505), ymin = 0.65, ymax = 1.35,
           alpha = 1,fill = "black") +
  ...
  annotate("rect", xmin = quantile(main_effect$b_loss_scaled, .17), xmax =  quantile(main_effect$b_loss_scaled, .83),  ymin = 2.65, ymax = 3.35,
           alpha = 1,fill ="#31A354") + 
  annotate("rect", xmin = quantile(main_effect$b_loss_scaled, .495), xmax =  quantile(main_effect$b_loss_scaled, .505),  ymin = 2.65, ymax = 3.35,
           alpha = 1,fill = "black") + 
  stat_pointinterval(position = position_dodge(width = .8), color = "black", alpha = .10, size = 0) +
...
radius = sample(spot_selection_data$step[spot_selection_data$step>-99 & spot_selection_data$step < 75], 50)
angle = sample(spot_selection_data$step[spot_selection_data$angle>-99], 50)
sim_spots_x = pull(traj[5,"x"]) + radius * cos(angle)
sim_spots_y = pull(traj[5,"y"]) + radius * sin(angle)
sim_spots = data.frame(sim_spots_x, sim_spots_y)
plot(sim_spots)

# extend to quadratic for easier plotting

# simulate evaluation points across whole subset
box = c(min(min(traj$x), min(sim_spots$sim_spots_x)) - 19.5, max(max(traj$x), max(sim_spots$sim_spots_x)) + 19.5,
        min(min(traj$y), min(sim_spots$sim_spots_y)) - 8.5, max(max(traj$y), max(sim_spots$sim_spots_y)) + 8.5)
box[2]-box[1]
box[4]-box[3]
...
#roughness_feature = kern(as.matrix(d[, 1:15]), exp(4.09), option = "roughness", ncol = 15)
roughness_feature = as.matrix(d[, 1])
sim_coords$roughness_feature = roughness_feature
...
s = softmax(1.98 * sim_coords$success_feature -.75 * sim_coords$loss_feature + .89 * sim_coords$social_feature + 1.46 * sim_coords$roughness_feature)
...
success_feature = ggplot() + 
  ...
  ylab("Northing")+
  ggtitle("Density successful spots")+
  labs(tag = "a")+
  xlab("Easting")+
  #theme_classic()+
...
loss_feature = ggplot() + 
  ...
  #scale_color_distiller()+
  ylab("Northing")+
  ggtitle("Density unsuccessful spots")+
...
prediction = ggplot() + 
  geom_point(data = sim_coords, aes(x = sim_x, y = sim_y, color = softmax * 50)) +
  geom_sf(data = profile, size = 2, color = "lightgrey") + 
  geom_path(data = traj, aes(x = x, y = y), color = "chartreuse")+
  geom_point(data = sim_spots, aes(x = sim_spots_x, y = sim_spots_y), size = 2, stroke = 1,color = "darkgrey", alpha = .75)+
  geom_point(data = traj[c(1, 2, 5),], aes(x = x, y = y), size = 3, stroke = 2, shape = 13, color = "black")+
  geom_point(data = traj[c(3, 4),], aes(x = x, y = y), size = 3, stroke = 2, shape = 8, color = "black")+
  geom_point(data = traj[6,], aes(x = x, y = y), size = 3, stroke = 2,color = "red", shape = 4)+
  coord_sf(xlim = c(box[1], box[2]), ylim = c(box[3],box[4]))+
  geom_point(data = coords %>% filter(between(X, box[1], box[2]) & between(Y, box[3], box[4])), aes(x = X, y = Y), size = 5, shape = 6, color = "darkgrey")+
  scale_color_viridis_c()+
  ylab("Latitude")+
  ggtitle("Choice Prediction")+
  labs(color = expression(italic(p)))+
  xlab("Longitude")+
  theme_classic()+
  theme(panel.background = element_rect(fill = NA),
