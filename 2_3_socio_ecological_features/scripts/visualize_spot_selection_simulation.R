...
visualize_spot_selection_simulation <- function(){
  ...
  # median split of observed social densities (same result if cutoff is computed for each lake)
  ...
  if (!file.exists("utils/data/processed_data/step_regression.rds")){
    model_steps_comparison = brms::brm(data = test_data, 
                                       formula = brmsformula(step ~ 1 + cond + time_since_last_success2+  
                                                               (1 + cond + time_since_last_success2| lake_id) + 
                                                               (1 + cond + time_since_last_success2| track_id) + 
                                                               (1 + cond + time_since_last_success2| participant_id), sigma ~ 1),
                                       family = lognormal(),
                                       chains = 4,
                                       cores = 4)
    test_data2 = plot_model_data %>% 
    ...
    model_angles_comparison = brms::brm(data = test_data2, 
                                        formula = brmsformula(absangle ~ 1 + cond + time_since_last_success2+
                                                                (1 + cond + time_since_last_success2| lake_id) + 
                                                                (1 + cond + time_since_last_success2| track_id) + 
                                                                (1 + cond + time_since_last_success2| participant_id), 
                                                              phi ~ 1 + cond +  time_since_last_success2 +
                                                                (1 + cond + time_since_last_success2| lake_id) + 
                                                                (1 + cond + time_since_last_success2| track_id) + 
                                                                (1 + cond + time_since_last_success2| participant_id)),
                                        family = Beta(),
  ...
  # table. Note that results are not exactly reproducible because social cutoff is affected by 
  # estimated social bandwidth, with can slightly change depending on the simulated alternative spots
  # analyses were rerun repeatedly to ensure that results are not affected by this random variation.
  summary(model_steps_comparison)
  summary(model_angles_comparison)
  
  ################################################################################
  # note that figures can look slightly different due to randomness in simulations.
  # simulations were run multiple times to ensure that results are not affected by this.
  steps_social = ggplot() +
    geom_vline(xintercept = 0, linetype = "dotted", color = "black", size = .6)+
...
panel_simulations = arrangeGrob(
  ...
  test_data = spot_selection_data %>% 
    ...
    mutate(n_spots = n()) %>% 
    filter(n_spots > 3) %>% # 4 spots needed for convex hull
