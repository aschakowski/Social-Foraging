...
sim_spatial_patch_leaving_model <- function(iter, 
  ...
  if(option == "flexible_bandwidth"){
    ...
    for (index in 1:stan_data$T){
      social_feature[index] = kern(d = sort(stan_data$social_matrix[index,])[1:15], h = exp(sim_id_effects_lambda$lambda + sim_lake_effects_lambda$lake_offset[sim_lake_effects_lambda$lake == stan_data$lakes[index]]), option = "social", ncol = 15) 
    }
  }
