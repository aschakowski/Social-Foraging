...
run_gut_simulations <- function(){
  ...
  # model locations
  model_locations = c("utils/data/processed_data/model_fit/patch_leaving_models/e_patch_leaving_model_bandwidth_fit.rds",
                      ...
                      "utils/data/processed_data/model_fit/patch_leaving_models/a_patch_leaving_model_baseline_fit.rds")
  
  # parameter extraction function
  extract_parameters_spatial <- function(model, option){
    ...
    sim_lake_effects_alpha = model %>% 
      spread_draws(v_lake[lake, parameter]) %>% 
      filter(parameter >= 10 & parameter <= 11) %>% 
    ...
    gc()
    # if flexible
    if (option == "flexible_bandwidth"){
      sim_id_effects_lambda = model %>% 
        spread_draws(lambda) %>% 
        summarize(lambda = mean(lambda))
      gc()
      sim_lake_effects_lambda = model %>% 
        spread_draws(v_lake[lake, parameter]) %>% 
        filter(parameter == 12) %>% 
        group_by(lake) %>% 
        summarize(lake_offset = mean(v_lake))
    ...
    if (option == "flexible_bandwidth"){
      return(list(sim_main_effects,
                  sim_main_effects_alpha,
                  sim_lake_effects,
                  sim_lake_effects_alpha,
                  sim_trip_effects,
                  sim_trip_effects_alpha,
                  sim_id_effects,
                  sim_id_effects_alpha,
                  sim_id_effects_lambda,
                  sim_lake_effects_lambda))
  ...
  extract_parameters_updating <- function(model){
  ...
  extract_parameters_patch_discovery <- function(model){
  ...
  extract_parameters_gut <- function(model){
  ...
  extract_parameters_baseline <- function(model){
  ...
  sim_spatial_patch_leaving_model <- function(iter, 
                                              stan_data, 
                                              parameters,
                                              option = "set_bandwidth"){
    
    if (option == "flexible_bandwidth"){
      sim_main_effects = parameters[[1]]
      sim_main_effects_alpha = parameters[[2]]
      sim_lake_effects = parameters[[3]]
      sim_lake_effects_alpha = parameters[[4]]
      sim_trip_effects = parameters[[5]]
      sim_trip_effects_alpha = parameters[[6]]
      sim_id_effects = parameters[[7]]
      sim_id_effects_alpha = parameters[[8]]
      sim_id_effects_lambda = parameters[[9]]
      sim_lake_effects_lambda = parameters[[10]]
    ...
    if(option == "flexible_bandwidth"){
      ...
      for (index in 1:stan_data$T){
        social_feature[index] = kern(d = sort(stan_data$social_matrix[index,])[1:15], h = exp(sim_id_effects_lambda$lambda + sim_lake_effects_lambda$lake_offset[sim_lake_effects_lambda$lake == stan_data$lakes[index]]), option = "social2", ncol = 15) 
      }
    }
    ...
    for (j in stan_data$trips){
      ...
      # spatial features
      social_feature_x = social_feature[start:end]
      ...
      for (s in unique(spot)){
        ...
        social_feature_tmp = social_feature_x[spot == s]
        ...
        while (leave == 0){
          ...
          if (cumulative_catch_tmp[t] == 0){
            ...
            predictors = c(1,
                           ...
                           global_reward[trip_length_counter-1],
                           local_reward[t],
        ...
        res[[s]] = data.frame(iter = iter,
                              ...
                              global_reward = global_reward[trip_length_counter-1],
  ...
  sim_patch_discovery_patch_leaving_model <- function(iter, 
    ...
    sim_main_effects = parameters[[1]]
    sim_lake_effects = parameters[[2]]
    sim_trip_effects = parameters[[3]]
    sim_id_effects = parameters[[4]]
  ...
  model = readRDS(model_locations[6])
  parameters = extract_parameters_baseline(model)
  rm(model)
  gc()
  start_time = Sys.time()
  for (i in 1:n_iter){
  ...
  # time wo catch model
  model = readRDS(model_locations[5])
  parameters = extract_parameters_gut(model)
  rm(model)
  gc()
  gc()
  ...
  model = readRDS(model_locations[4])
  parameters = extract_parameters_patch_discovery(model)
  rm(model)
  start_time = Sys.time()
  for (i in 1:n_iter){
  ...
  model = readRDS(model_locations[3])
  parameters = extract_parameters_updating(model)
  rm(model)
  start_time = Sys.time()
  for (i in 1:n_iter){
  ...
  # spatial features # change simulation function to include flexible bandwidth.
  sim_res_spatial_features = list()
  model = readRDS(model_locations[1])
  parameters = extract_parameters_spatial(model, option = "flexible_bandwidth")
  rm(model)
  start_time = Sys.time()
  for (i in 1:n_iter){
