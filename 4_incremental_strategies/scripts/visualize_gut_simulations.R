...
for (model in 1:5){
  ...
  plots[[model]] = ggplot(data = plot_data, aes(x = gut_data_mean, y = gut_sim_mean)) + 
    ...
    annotate("text", x = 100, y = 100, label = paste("r = ", rd(correlation$mean, digits = 2), " [", rd(correlation$lower, digits = 2), " ,", rd(correlation$upper, digits = 2), "]", sep = ""), 
             size = 6) + 
    ggtitle(title[model])+
    coord_fixed(ratio = 1,
                ylim = c(0, 150),
...
for (model in 1:5){
  ...
  plots_lake[[model]] = ggplot(data = plot_data, aes(x = gut_data_mean, y = gut_sim_mean)) + 
    ...
    geom_errorbar(aes(xmin = lowerx, xmax = upperx), alpha = .2)+
    geom_errorbar(aes(ymin = lowery, ymax = uppery), alpha = .2) + 
    annotate("text", x = 30, y = 30, label = paste("r = ", rd(correlation$mean, digits = 2), " [", rd(correlation$lower, digits = 2), " ,", rd(correlation$upper, digits = 2), "]", sep = ""), 
             size = 6) + 
    ggtitle(title[model])+
    coord_fixed(ratio = 1,
                ylim = c(0, 40),
...
#               sd_lake__Intercept,
#               sigma) %>% 
#  mutate(icc_id = sd_id__Intercept^2 / (sd_lake__Intercept^2 + sd_id__Intercept^2 + sd_trip__Intercept^2 + sigma^2),
#         icc_lake = sd_lake__Intercept^2 / (sd_lake__Intercept^2 + sd_id__Intercept^2 + sd_trip__Intercept^2 + sigma^2)) %>% 
#  tidyr::pivot_longer(8:9) %>% 
...
ggsave(panel_delta_gut_predictions, file = "4_incremental_strategies/output/panel_patch_leaving_delta_gut_predictions.png", 
       width = 21,height = 6, bg = "white", dpi = 300)
