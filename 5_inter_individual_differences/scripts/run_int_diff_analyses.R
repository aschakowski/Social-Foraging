...
# standardize ID effects (using the same SDs as for standardization of main effects)
...
scale_features = fitted_features %>% 
  ...
  dplyr::summarize(sd_social = sd(social_feature),
  ...
  ungroup() %>% 
  group_by(lake) %>% 
  dplyr::summarize(sd_social = mean(sd_social),
...
if (!file.exists("utils/data/processed_data/ind_regression_tmp.rds")){
  id_effects = tmp_model %>% 
    spread_draws(v_id[id_index, parameter])
  key = identifiers %>% group_by(participant_id) %>% slice(1) %>% dplyr::select(participant_id, id_index = model_participant_id)
  id_effects = left_join(id_effects, key)
  dem_dat = catch_data %>% mutate(age = 2023-year_of_birth) %>% dplyr::select(participant_id, age, sex) %>% 
    group_by(participant_id) %>% 
    slice(1)
  survey_data = read_csv("utils/data/raw_data/survey_data.csv")
  skill = survey_data %>% dplyr::select(participant_id, angling_skill)
  dem_dat = left_join(dem_dat, skill)
  id_effects = left_join(id_effects, dem_dat)
  id_effects = id_effects[!is.na(id_effects$age),]
  id_effects = id_effects[!is.na(id_effects$angling_skill),]
  id_effects = id_effects[!is.na(id_effects$sex),]
...
end_time - start_time
regression_list = do.call(rbind, regression_list)
parameter_key = data.frame(parameter = 1:11, name = c("Baseline", "Fish\nDiscovery", "Time w/o\nCatch", "Waiting  Time\nLocal", "Waiting Time\nGlobal", "Success", "Loss", "Roughness", "Social", "Updating\nGlobal", "Updating\nLocal"))
...
avg_catch_prediction = ggplot() + 
  ...
  stat_summary(data = lake_data, aes(x = avg_catch/1000, y = gut_data*10/60, color = catch),fun.data = mean_se, fun.args = list(mult = 2), size = .1) +
...
ratio_successful_prediction = ggplot() + 
  ...
  stat_summary(data = lake_data, aes(x = ratio_successful*100, y = gut_data*10/60, color = catch),fun.data = mean_se, fun.args = list(mult = 2), size = .1) +
...
catch_data = fread("utils/data/raw_data/catch_data.csv")
avg_catch = catch_data %>% group_by(day, year) %>% summarize(avg_catch = mean(catch))
percent_successful = spot_selection_data %>% group_by(day, year) %>% summarize(percent_successful = mean(Return))
travel_time = spot_selection_data %>% 
  group_by(day, year, camera_id, participant_id) %>% 
  filter(Return == 1) %>% 
  mutate(travel_time = time_start - lag(time_end)) %>% 
  summarize(travel_time = mean(travel_time, na.rm = T)) %>% 
  filter(!is.na(travel_time))
travel_time = left_join(left_join(travel_time, percent_successful), avg_catch)

# add spatial spatial features
fitted_features = readRDS(file = "utils/data/processed_data/fitted_features.rds")
avg_features = fitted_features %>% group_by(day, year, camera_id) %>% summarize(avg_success = mean(success_feature, na.rm = T),
...
travel_time$percent_successful = travel_time$percent_successful*100
...
################################################################################
# REVISION: Make figure comparing ML and JL solutions
################################################################################
# REVISION: compare group level predictor estimates: 
# extract group level predictor regression weights and save for later comparison
# with analytic method
m = readRDS("utils/data/processed_data/model_fit/spot_selection_models/group_level_predictors_lake_ft1.rds")
sds = spatial_model %>% spread_draws(sigma_id[parameter]) %>% 
  group_by(parameter) %>% 
  summarize(sd = mean(sigma_id))

regression_plot_spatial_grp = m %>% spread_draws(Beta_id[predictor, parameter]) %>% 
  left_join(sds) %>% 
  filter(parameter <=4) %>% 
  mutate(name = ifelse(parameter == 1, "Social", ifelse(parameter == 2, "Roughness", ifelse(parameter == 3, "Successful", "Unsuccessful"))),
         Beta_id = ifelse(predictor == 2, -Beta_id, Beta_id)) %>% 
  mutate(predictor = ifelse(predictor == 1, "age", ifelse(predictor == 2, "gender\n(ref. = f)", "skill"))) %>% 
  mutate(name = factor(name, levels = rev(c("Successful", "Unsuccessful", "Social", "Roughness")))) %>%
  ggplot(aes(x = name, y = Beta_id, group = predictor, color = predictor)) + 
  stat_pointinterval(position = position_dodge(.4),size = 1) + 
  scale_size_continuous(range = c(3, 10))+
  scale_color_npg(breaks = c("skill", "gender\n(ref. = f)", "age")) + 
  coord_flip(ylim = c(-1.5, 1.5)) +
  scale_y_continuous(breaks = seq(from = -2.5, to = 2.5, by = .5))+
  geom_hline(yintercept = 0, linetype = "dotted") + 
  ylab("Posterior Estimate (95% CrI)\nInd. Feature Weight ~ Predictor") + 
  labs(tag = "A", color = "Predictor")+
  ggtitle("Full Model")+
  theme(panel.background = element_rect(fill = NA),
        axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        ...
        text = element_text(family = "sans"),
        legend.box.margin = margin(10, 10, 10, 10))
regression_plot_spatial_grp

regression_list_plot = readRDS("utils/data/processed_data/ind_regression_spatial.rds")
regression_plot_spatial_grp_comp = regression_list_plot %>% 
  filter(name %in% c("Social", "Roughness", "Successful", "Unsuccessful")) %>% 
  mutate(name = factor(name, levels = rev(c("Successful", "Unsuccessful", "Social", "Roughness")))) %>% 
  ggplot(aes(x = name, y = Estimate, group = Predictor, color = Predictor)) + 
  stat_pointinterval(position = position_dodge(.4),size = 1) + 
  ...
  theme(panel.background = element_rect(fill = NA),
        ...
        text = element_text(family = "sans"),
        legend.box.margin = margin(10, 10, 10, 10))
regression_plot_spatial_grp_comp

# lake level predictors
regression_plot_spatial_grp = m2 %>% spread_draws(Beta_id[predictor, parameter]) %>% 
  left_join(sds) %>% 
  filter(parameter <=4) %>% 
  mutate(name = ifelse(parameter == 1, "Social", ifelse(parameter == 2, "Roughness", ifelse(parameter == 3, "Successful", "Unsuccessful"))),
         Beta_id = ifelse(predictor == 2, -Beta_id, Beta_id)) %>% 
  mutate(predictor = ifelse(predictor == 1, "age", ifelse(predictor == 2, "gender\n(ref. = f)", "skill"))) %>% 
  mutate(name = factor(name, levels = rev(c("Successful", "Unsuccessful", "Social", "Roughness")))) %>%
  ggplot(aes(x = name, y = Beta_id/sd, group = predictor, color = predictor)) + 
  stat_pointinterval(position = position_dodge(.4),size = 1) + 
  scale_size_continuous(range = c(3, 10))+
  scale_color_npg(breaks = c("skill", "gender\n(ref. = f)", "age")) + 
...
regression_plot_spatial_eco_unstandardized = readRDS("utils/data/processed_data/eco_regression_spatial_unstandardized.rds")
regression_plot_spatial_eco_grp = regression_list_plot_eco_unstandardized %>% 
  filter(name %in% c("Social", "Roughness", "Successful", "Unsuccessful")) %>% 
  mutate(name = factor(name, levels = (c("Roughness", "Social", "Unsuccessful", "Successful")))) %>% 
  ggplot(aes(x = name, y = Estimate, group = Predictor, color = Predictor)) + 
  stat_pointinterval(position = position_dodge(.4),size = 1) + 
...
panel_grp_comp = arrangeGrob(
  grobs = list(regression_plot_spatial_grp,
               regression_plot_spatial_grp_comp),
  widths = c(1.2, 1),
  layout_matrix = rbind(c(1, 2))
)
ggsave(panel_grp_comp, file = "5_inter_individual_differences/output/panel_grp_comp.png", 
       width = 12, height = 5, bg = "white", dpi = 600, units = "cm")
ggsave(panel_grp_comp, file = "5_inter_individual_differences/output/panel_grp_comp.pdf", 
       width = 12, height = 5, bg = "white", dpi = 600, units = "cm")
ggsave(panel_grp_comp, file = "5_inter_individual_differences/output/panel_grp_comp.svg", 
       width = 12, height = 5, bg = "white", dpi = 600, units = "cm")
ggsave(panel_grp_comp, file = "5_inter_individual_differences/output/panel_grp_comp.tiff", 
       width = 12, height = 5, bg = "white", dpi = 600, units = "cm")
