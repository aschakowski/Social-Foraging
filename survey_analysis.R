...
################################################################################
# best describe oneself during fishing experience
#self_description = survey_data %>%
#  group_by(participant_id) %>% 
#  slice(1) %>% 
#  ggplot(aes(x = P7_Q12, group = as.factor(sex), fill = as.factor(sex))) + 
#  coord_cartesian(xlim = c(1,4))+
#  scale_x_continuous(labels=c("Often uncertain",
#                              "Some understanding",
#                              "Good understanding",
#                              "Facilitator\nand Teacher"), breaks=1:4) +
#  geom_histogram(position = position_dodge()) + 
#  xlab("")+
#  ggtitle("Self Description Experience")+
#  theme_minimal() + 
#  labs(fill = "Gender", tag = "a") +
#  ylab("Avg. Response") + 
#  xlab("Item") + 
...
#  group_by(participant_id) %>% 
#  slice(1) %>% 
#  ggplot(aes(x = angling_skill, group = as.factor(sex), fill = as.factor(sex))) + 
#  coord_cartesian(xlim = c(1,5))+
#  scale_x_continuous(labels=c("1", "2", "3", "4", "5"), breaks=1:5) +
#  geom_histogram(position = position_dodge()) + 
#  xlab("")+
#  ggtitle("Angling Skill (N = 71)")+
#  theme_minimal() + 
#  labs(fill = "Gender", tag = "b") +
#  ylab("Avg. Response") + 
#  xlab("Item") + 
#  theme(legend.position = c(.9, .8),
#        axis.title.x = element_text(size = 20),
